...
{{--    header navbar navbar-expand-lg bg-light shadow-sm shadow-dark-mode-none fixed-top--}}
    ...
    <nav class="container mt-lg-4 pt-5" aria-label="breadcrumb">
        <ol class="breadcrumb mb-0 pt-5">
            <li class="breadcrumb-item">
                <a href="index.html"><i class="bx bx-home-alt fs-lg me-1"></i>Главаня</a>
            </li>
    ...
    <section class="container mt-4 mb-2 mb-md-4 mb-lg-5 pt-lg-2 pb-5">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4 pb-1 pb-md-3">
            <h2 class="display-1 mb-0"><span class="text-gradient-primary">Самые честные новости</span></h2>
        </div>
        ...
        <div class="row">
            ...
            <aside class="col-xl-3 col-lg-4">
                <div class="offcanvas-lg offcanvas-end" id="blog-sidebar" tabindex="-1">
                    ...
                    <!-- Body -->
                    @livewire('main-home.load-popular-posts')
                </div>
            </aside>
        </div>
    </section>
...
<script>
    ...
    channel.bind('my-event', function(data) {
        if (!("Notification" in window)) {
            ...
            // Если разрешение уже предоставлено, то все готово к отправке уведомлений
        } else if (Notification.permission !== "denied") {
            // Запрашиваем разрешение у пользователя
            Notification.requestPermission().then(function (permission) {
                if (permission === "granted") {
                    // Если разрешение получено, то все готово к отправке уведомлений
                }
            });
        }
        if (Notification.permission === "granted") {
            var notification = new Notification("Новый пост от @Laravel", {
                body: "Section 1.10vb gghgfhfgwritten by Cicero"
            });

            // Обработчик клика на уведомлении
            notification.onclick = function () {
                window.focus(); // При клике на уведомлении открываем окно, если оно было свернуто
                notification.close(); // Закрываем уведомление
                // Добавьте здесь код, который должен выполниться при клике на уведомлении
            };
        }
    });
</script>
